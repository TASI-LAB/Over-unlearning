...
# from resnet import ResNet18_CIFAR100
...
class CIFAR100:
    def __init__(self):
        ...
        x_train, x_test = np.load('train_test_data/Cifar100/X_train.npy'), np.load('train_test_data/Cifar100/X_test.npy')
        x_train = np.array(x_train,dtype=np.float32)
        x_valid = np.load('train_test_data/Cifar100/X_valid.npy')
        ...
        y_train, y_test = np.load('train_test_data/Cifar100/y_train_fine.npy'), np.load('train_test_data/Cifar100/y_test_fine.npy')
        y_valid = np.load('train_test_data/Cifar100/y_valid_fine.npy')
        y_train = y_train.astype(int)
        y_train_one_hot = np.eye(100)[y_train]
        y_valid = y_valid.astype(int)
        y_valid_one_hot = np.eye(100)[y_valid]
...
class CIFAR100Model:
    def __init__(self, restore=None, session=None, use_log=False, network='resnet18'):
        model = None
        self.num_channels = 3
        self.image_size = 32
        self.num_labels = 100
        if network == 'vgg16':
            ...
            print("get resnet18 !!!!!!!!!!")
            model = ResNet18_CIFAR100(restore, session, use_log)
        elif network == 'vgg19':
            print("get vgg19 !!!!!!!!!!")
