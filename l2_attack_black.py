...
MAX_ITERATIONS = 10000   # number of iterations to perform gradient descent org:10000
...
class BlackBoxL2:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
        ...
        if self.use_resize:
            self.modifier = tf.compat.v1.placeholder(tf.float32, shape=(None, None, None, None))
            # scaled up image
            self.scaled_modifier = tf.image.resize(self.modifier, [image_size, image_size])
            # operator used for resizing image
            self.resize_size_x = tf.compat.v1.placeholder(tf.int32)
            self.resize_size_y = tf.compat.v1.placeholder(tf.int32)
            self.resize_input = tf.compat.v1.placeholder(tf.float32, shape=(1, None, None, None))
            self.resize_op = tf.image.resize(self.resize_input, [self.resize_size_x, self.resize_size_y])
        else:
            self.modifier = tf.compat.v1.placeholder(tf.float32, shape=(None, image_size, image_size, num_channels))
            # no resize
        ...
        self.assign_timg = tf.compat.v1.placeholder(tf.float32, single_shape)
        self.assign_tlab = tf.compat.v1.placeholder(tf.float32, num_labels)
        self.assign_const = tf.compat.v1.placeholder(tf.float32)
        ...
        if self.TARGETED:
            if use_log:
                ...
                loss1 = tf.maximum(0.0, tf.compat.v1.log(self.other + 1e-30) - tf.compat.v1.log(self.real + 1e-30))
            else:
            ...
            if use_log:
                # loss1 = tf.compat.v1.log(self.real)
                loss1 = tf.maximum(0.0, tf.compat.v1.log(self.real + 1e-30) - tf.compat.v1.log(self.other + 1e-30))
            else:
    ...
    def get_new_prob(self, prev_modifier, gen_double = False):
        ...
        for i in range(prev_modifier.shape[2]):
            ...
            if gen_double:
                ...
                size = tuple((np.array(image_pool.size) * 2.0).astype(int))
                prob[:,:,i] = numpy.array(Image.fromarray(image_pool).resize(size, PIL.Image.BICUBIC))
            else:
                prob[:,:,i] = image_pool
        prob /= np.sum(prob)
        return prob
    ...
    def attack(self, imgs, targets, just_within_edge):
        """
        ...
        for i in range(0,len(imgs)):
            print('tick',i)
            r.extend(self.attack_batch(imgs[i], targets[i],just_within_edge))
        return np.array(r)

    # only accepts 1 image at a time. Batch is used for gradient evaluations at different points
    def attack_batch(self, img, lab, just_within_edge, orglab = None):
        """
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.start_iter, self.MAX_ITERATIONS):
                ...
                # adversarial_predict = self.model.predict(nimg)
                ...
                # print("inner adversarial probabilities:", adversarial_prob[-1:-6:-1])
                # print("inner adversarial classification:", adversarial_class[-1:-6:-1])
                ...
                if not just_within_edge:
                    ...
                    if self.TARGETED:
                        if compare(score, np.argmax(lab)):
                            print("[STATS][L3](First valid attack found!) iter = {}, cost = {}, time = {:.3f}, size = {}, loss = {:.5g}, loss1 = {:.5g}, loss2 = {:.5g}, l2 = {:.5g}".format(iteration, eval_costs, train_timer, self.real_modifier.shape, l, loss1, loss2, l2))
                            sys.stdout.flush()
                            print('find success attack')
                            o_bestl2 = bestl2
                            o_bestscore = bestscore
                            o_bestattack = pastattack
                            o_best_const = pastconst
                            break
                        if l2 > bestl2 and compare(score, np.argmax(orglab)):
                            bestl2 = l2
                            bestscore = np.argmax(score)
                            pastattack = nimg
                            pastconst = CONST
                        ...
                        if compare(score, np.argmax(lab)):
                            ...
                            o_best_const = pastconst
                            break
                        if l2 > bestl2 and not compare(score, np.argmax(lab)):
                            bestl2 = l2
                            bestscore = np.argmax(score)
                            pastattack = nimg
                            pastconst = CONST
